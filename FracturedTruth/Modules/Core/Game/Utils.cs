...
using FracturedTruth.DataHandling.FinalAntiCheat.Core;
using FracturedTruth.DataHandling.XtremeGameData;
...
namespace FracturedTruth.Modules.Core.Game;
...
public static class Utils
{
    ...
    {
        var folder = Directory.CreateDirectory($"{Application.persistentDataPath}/FracturedTruth/Logs");
    ...
    public static void DumpLog(bool popup = false)
    {
        ...
        if (popup)
            HudManager.Instance.ShowPopUp(string.Format(GetString("Message.DumpfileSaved"),
                $"FracturedTruth - v{Main.DisplayedVersion}-{t}.log"));
        else
    ...
    public static string CopyLog(string path)
    {
        ...
        var t = DateTime.Now.ToString("yyyy-MM-dd_HH.mm.ss");
        var fileName = $"{f}FracturedTruth-v{Main.DisplayedVersion}-{t}.log";
    ...
    public static Texture2D LoadTextureFromResources(string file)
    {
        ...
        InDLL:
        /*path = "FracturedTruth.Resources.Images." + file;
